...
/**
 * Adapts a compiler {@link com.olo.executiontree.ExecutionTreeNode} to the
...
public final class CompilerNodeAdapter extends ExecutionTreeNode {
    ...
    private final com.olo.executiontree.ExecutionTreeNode delegate;

    public CompilerNodeAdapter(com.olo.executiontree.ExecutionTreeNode delegate) {
        super(
            delegate.getId(),
            delegate.getName(),
    ...
    private static NodeType toTreeNodeType(com.olo.executiontree.NodeType t) {
    ...
    private static List<ExecutionTreeNode> wrapChildren(List<com.olo.executiontree.ExecutionTreeNode> children) {
        if (children == null || children.isEmpty()) return List.of();
        List<ExecutionTreeNode> out = new ArrayList<>(children.size());
        for (com.olo.executiontree.ExecutionTreeNode c : children) {
            out.add(new CompilerNodeAdapter(c));
        }
        return out;
    }

    private static String getPluginRef(com.olo.executiontree.ExecutionTreeNode n) {
