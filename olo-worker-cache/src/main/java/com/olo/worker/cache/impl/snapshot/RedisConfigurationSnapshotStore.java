package com.olo.worker.cache.impl.snapshot;
...
import com.olo.configuration.impl.snapshot.CorePipelinesConnections;
import com.olo.configuration.impl.snapshot.SectionedBatch;
import com.olo.configuration.impl.snapshot.SnapshotChecksum;
import com.olo.configuration.snapshot.BlockMetadata;
...
public final class RedisConfigurationSnapshotStore implements ConfigurationSnapshotStore, AutoCloseable {
  ...
  public SnapshotMetadata getMeta(String region) {
    ...
    try {
      ...
      withConnection(conn -> {
        ...
        String globalMetaKey = com.olo.configuration.RedisKeys.configPrefix() + ":meta";
        out[0] = cmd.get(globalMetaKey);
      });
      String json = out[0];
      if (json == null || json.isBlank()) return null;
      if (isOverLimit(json, com.olo.configuration.RedisKeys.configPrefix() + ":meta")) return null;
  ...
  public void put(String region, ConfigurationSnapshot snapshot) {
    ...
    String coreKey = com.olo.configuration.RedisKeys.configPrefix() + ":core";
    try {
      ...
      String existingGlobalMetaJson = getString(com.olo.configuration.RedisKeys.configPrefix() + ":meta");
      ...
      if (isOverLimit(coreJson, coreKey)
          || isOverLimit(globalMetaJson, com.olo.configuration.RedisKeys.configPrefix() + ":meta")) {
      ...
      withConnection(conn -> {
        ...
        try {
          ...
          cmd.set(com.olo.configuration.RedisKeys.configPrefix() + ":meta", globalMetaJson);
          cmd.exec();
        } catch (Exception e) {
          cmd.discard();
  ...
  public void putPipelines(String region, Map<String, Object> pipelines) {
    ...
    String pipelinesKey = com.olo.configuration.RedisKeys.configPrefix() + ":pipelines:" + region;
    try {
  ...
  public ConfigurationSnapshot getCore(String region) {
    ...
    try {
      final String[] out = new String[1];
      String key = com.olo.configuration.RedisKeys.configPrefix() + ":core";
      withConnection(conn -> {
        RedisCommands<String, String> cmd = conn.sync();
        out[0] = cmd.get(key);
      });
  ...
  public Map<String, Object> getPipelines(String region) {
    ...
    try {
      final String[] out = new String[1];
      String key = com.olo.configuration.RedisKeys.configPrefix() + ":pipelines:" + region;
      withConnection(conn -> {
        RedisCommands<String, String> cmd = conn.sync();
        out[0] = cmd.get(key);
      });
  ...
  public Map<String, Object> getConnections(String region) {
    ...
    try {
      final String[] out = new String[1];
      String key = com.olo.configuration.RedisKeys.configPrefix() + ":connections:" + region;
      withConnection(conn -> {
        RedisCommands<String, String> cmd = conn.sync();
        out[0] = cmd.get(key);
      });
  ...
  public CorePipelinesConnections getCorePipelinesConnectionsBatch(String region) {
    ...
    String coreKey = com.olo.configuration.RedisKeys.configPrefix() + ":core";
    String pipelinesKey = com.olo.configuration.RedisKeys.configPrefix() + ":pipelines:" + region;
    String connectionsKey = com.olo.configuration.RedisKeys.configPrefix() + ":connections:" + region;
    try {
  ...
  public SectionedBatch getSectionedBatch(String region) {
    if (region == null || region.isEmpty()) return null;
    String metaKey = com.olo.configuration.RedisKeys.configPrefix() + ":meta";
    String coreKey = com.olo.configuration.RedisKeys.configPrefix() + ":core";
    String pipelinesKey = com.olo.configuration.RedisKeys.configPrefix() + ":pipelines:" + region;
    String connectionsKey = com.olo.configuration.RedisKeys.configPrefix() + ":connections:" + region;
    try {
  ...
  private static int getMaxRedisValueBytes() {
    com.olo.configuration.Configuration c = ConfigurationProvider.get();
    if (c == null) return 0;
