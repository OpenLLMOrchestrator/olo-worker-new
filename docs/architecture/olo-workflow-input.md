...
- **Define the input schema**: `com.olo.workflow.input.model.OloWorkerRequest`
...
- **`com.olo.workflow.input.parser.OloPayloadParser`**
...
- **`com.olo.workflow.input.parser.VersionStrategy`**
...
- **`com.olo.workflow.input.validation.OloPayloadValidator`**
  - **`validate(OloWorkerRequest request)`** — throws `OloValidationException` if invalid.
- **`com.olo.workflow.input.validation.OloValidationException`**
  - **`getErrors()`** — list of validation error messages.
- **`com.olo.workflow.input.util.InputResolver`**
