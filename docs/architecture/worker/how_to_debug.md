...
- **Temporal** server running (e.g. `localhost:7233`) and namespace configured (e.g. `default`).
...
The **`routing.pipeline`** value must match a **task queue** the worker is listening on. Task queues are derived from config and follow the form **`olo.<region>.<pipelineId>`**. Examples from the debug schema:

- `olo.default.default-pipeline` (debug pipeline; allowed tenant `tenant-a`)
- `olo.default.default-pipeline-2`
- `olo.us-east.us-east-pipeline-1` (region `us-east`)

Use a pipeline that exists in your Redis/DB snapshot (e.g. after running `configuration/debug/ensure-schema.sql` or `schema-and-data.sql`).
...
2. Connects to the Temporal server (default `localhost:7233`, namespace `default`).
3. **Starts the worker** (same process) polling the task queue `olo.default.default-pipeline`.
...
```bash
./gradlew :olo-worker:test --tests "com.olo.worker.WorkflowInputDebugIntegrationTest"
```
...
To test input format and workflow start from tests under **`olo-worker/src/test/java/com/olo/worker`**:
...
See the integration test **`WorkflowInputDebugIntegrationTest`** in `olo-worker/src/test/java/com/olo/worker/WorkflowInputDebugIntegrationTest.java`: it loads input from `configuration/debug/workflow/sample-default-pipeline.json`, runs bootstrap, starts a workflow using Temporal’s test environment, sets `olo.debug.dump.input.dir` to `build/debug-dump`, and asserts that the dump file contains `workflowInput` and `localContext`.
