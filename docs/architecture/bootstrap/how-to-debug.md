...
|-----------|---------|---------|
| `olo.region` / `OLO_REGION` | Worker region (comma-separated list of regions this instance serves) | `default` or `us-east` |
...
- Redis must contain **snapshot metadata** and **core**:
  - `olo:config:meta`
  - `olo:config:core`
...
Individual SQL scripts are also under **`configuration/sql/`** (002, 003, 004) and **`olo-worker-configuration/src/main/resources/schema/`** if you need to run them separately.
...
3. Run the integration test:
   ...
   The test runs **only** the bootstrap dump test (it is excluded from the normal `test` task).
...
```bash
./gradlew :olo-worker:integrationTest -Dolo.bootstrap.dump.output=path/to/global-context.json
```
