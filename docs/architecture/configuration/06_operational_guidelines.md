...
- **Local config key**: `olo.region` — comma-separated list of regions this instance serves. Default: `default`.
- **ENV override**: `OLO_REGION=default,us-east`
...
Implement by checking **Regions.getRegions(Configuration)** or the configured `olo.region` list before using **TenantRegionResolver.getRegion(tenantId)** to resolve config; if the resolved region is not in the worker’s served list, reject or redirect.
...
- **`olo.region`** — Comma-separated regions this instance serves (one or more; each has its own snapshot)
...
| **`Regions.getRegions(Configuration)`** | List of regions from config key `olo.region`. |
