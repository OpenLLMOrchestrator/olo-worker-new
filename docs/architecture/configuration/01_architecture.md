...
Use **Bootstrap.run()** at worker startup. Use **ConfigSnapshotBuilder.buildAndStore(region)** (in `com.olo.configuration.impl.snapshot`) in the admin service after updating DB. Implementation classes live under **`com.olo.configuration.impl`** and subpackages (`impl.connection`, `impl.config`, `impl.refresh`, `impl.snapshot`, `impl.source`, `impl.region`); API types remain in `configuration`, `configuration.snapshot`, `configuration.source`, and `configuration.region`.
...
**Package layout:** API types stay in `com.olo.configuration`, `com.olo.configuration.snapshot`, `com.olo.configuration.source`, and `com.olo.configuration.region`. All **implementations** live under **`com.olo.configuration.impl`** and small subpackages.

```
com.olo.configuration
├── Bootstrap.java
├── Configuration.java
...
│
└── impl/                            # implementations (com.olo.configuration.impl)
