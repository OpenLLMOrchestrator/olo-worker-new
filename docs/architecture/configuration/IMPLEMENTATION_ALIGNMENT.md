...
| **Bootstrap flow** | `Bootstrap.run()`: defaults → env → wait for Redis + snapshot → load snapshot → set ConfigurationProvider; tenant region load; optional refresh + Pub/Sub + TenantRegionRefreshScheduler | Order matches doc. |
